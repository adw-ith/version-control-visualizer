...
export default function CommitDetailsPage() {
  ...
  const { vcs, owner, repo, commitSha } = params;
  ...
  const fetchCommitDetails = async () => {
    try {
      const res = await fetch(
        `/api/commit-details?vcs=${vcs}&owner=${owner}&repo=${repo}&sha=${commitSha}`
      );
  ...
  return (
    <div className="p-6 mx-auto bg-white text-gray-900 shadow-md border border-gray-300">
      <h1 className="text-2xl font-bold mb-4">
        Commit Details: <span className="text-blue-600">{commitSha}</span>
      </h1>
